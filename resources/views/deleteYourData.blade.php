...
<header>
    ...
    <nav class="navbar">
        <ul>
            ...
            <li><a href="{{ route('DeleteYourDataEn') }}">En</a></li>
        </ul>
    </nav>
...
<section class="home" id="home">
    <div class="heading"><span>حذف بياناتك</span></div>
    <div class="box-container">
        <div class="box">

            <p>يمكنك طلب حذف بياناتك في منصتنا بارسالك هذا النموذج:</p>
            ...
            <div>
                <form method="POST" action="{{ route('sendDeleteUserData') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <p>الاسم كامل: @error('Name') <small id="ErrorName" class="form-text text-danger">{{ $message }}</small> @enderror</p>
                        <input type="text" id="Name" class="form-control" name="Name" required autofocus>
                    </div>
                    <div class="mb-3">
                        <p>رقم الجوال: @error('Phone') <small id="ErroPhone" class="form-text text-danger">{{ $message }}</small> @enderror</p>
                    ...
                    <button id="button" type="submit" class="MyButton">ارسال</button>
                </form>
    ...
    @if(!empty(Session::get('send_successfully')) && Session::get('send_successfully') == 2)
        <script>
            alert("تم ارسال الطلب بنجاح, سوف نرسل طلب تاكيد على حسابك");
        </script>
    @endif
    @if(!empty(Session::get('error_send')) && Session::get('error_send') == 3)
        <script>
            alert("لقد قمت بتقديم طلب مسبقاً");
        </script>
    @endif
</section>
