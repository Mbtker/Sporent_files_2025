<div class="boardView_details">
    <div class="boardView">
    <!-- Table -->
    <table style="width: 100%">
        <thead>
        <tr>
            <td style="text-align: center; width: 40px">#</td>
            <td style="text-align: center; width: 150px">{{ __('messages.TeamLogo') }}</td>
            <td style="min-width: 100px">{{ __('messages.Name') }}</td>
            <td style="text-align: center">{{ __('messages.CaptainName') }}</td>
            <td style="text-align: center; display: none">CaptainId</td>
            <td style="text-align: center">{{ __('messages.TeamLeader') }}</td>
            <td style="text-align: center; display: none">TeamLeaderId</td>
            <td style="text-align: center">{{ __('messages.CityName') }}</td>
            <td style="text-align: center">{{ __('messages.Location') }}</td>
            <td style="text-align: center">{{ __('messages.ChatGroup') }}</td>
            <td style="text-align: center">{{ __('messages.EPaymentAvailable') }}</td>
            <td style="text-align: center; width: 95px">{{ __('messages.Status') }}</td>
            <td style="text-align: center; width: 30px">{{ __('messages.Action') }}</td>
        </tr>
        </thead>
        <tbody id="table_data">
        @if (isset($MyArray))
            @foreach($MyArray as $Item)
                <tr>
                    <td style="text-align: center">{{ $Item->{'Id'} }}</td>
                    <td style="text-align: center"><img src="{{ $Item->{'Logo'} }}"  alt="team_logo"/></td>
                    <td>@if(app()->getLocale() == 'en') {{ $Item->{'NameEn'} }} @else {{ $Item->{'NameAr'} }} @endif</td>
                    <td><a id="CaptainDetails" style="text-decoration: none; color: black" href="#">{{ $Item->{'CaptainName'} }}</a></td>
                    <td style="display: none">{{ $Item->{'CaptainId'} }}</td>
                    <td><a id="TeamLeaderDetails" style="text-decoration: none; color: black" href="#">{{ $Item->{'TeamLeader'} }}</a></td>
                    <td style="display: none">{{ $Item->{'TeamLeaderId'} }}</td>
                    <td style="text-align: center">@if($Item->{'CityName'} == null) {{ '-' }} @else {{ $Item->{'CityName'} }} @endif</td>
                    <td style="text-align: center">
                        <a href="https://www.google.com/maps/place/{{ $Item->{'Location'} }}" target="_blank" style="text-decoration: none; color: black; font-size: 18px"><i class="fad fa-map-marked map_icon"></i></a>
                    </td>
                    <td style="text-align: center">@if($Item->{'ChatUrl'} == null) {{ '-' }} @else <a href="{{ $Item->{'ChatUrl'} }}" target="_blank" style="text-decoration: none; color: #469163; font-size: 22px"><i class="fab fa-whatsapp"></i></a> @endif</td>
                    <td style="text-align: center">{{ $Item->{'EPaymentAvailable'} }}</td>
                    <td style="text-align: center">@if ($Item->{'Status'} == '1') {{ __('messages.Active') }} @else {{ __('messages.Inactive') }} @endif</td>
                    <td style="text-align: center; display: none">{{ $Item->{'Status'} }}</td>
                    <td class="text-align: center">
                        <a class="Details" href="#" style="text-decoration: none; color: #7e7e7e; margin-inline-end: 10px"><i class="fal fa-file-alt"></i></a>
                        <a class="Edit" href="#" style="text-decoration: none; color: #7e7e7e;"><i class="fad fa-edit"></i></a>
                    </td>
                </tr>
            @endforeach
        @endif
        </tbody>
    </table>

    @if (!isset($MyArray) || count($MyArray) == 0)
        <div class="NoData">
            <p>{{ __('messages.NoDate') }}</p>
        </div>
    @endif
<!-- To show no data if array count is 0 -->

        @if(!$iSFromSearch)
            <div style="margin: 8px; font-size: 11px">
                @if (isset($MyArray))
                    {{ $MyArray->onEachSide(1)->links() }}
                @endif
            </div>
        @endif

</div>

</div>
