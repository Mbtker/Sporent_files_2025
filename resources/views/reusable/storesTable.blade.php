<div class="boardView_details">
    ...
    <table style="width: 100%">
        ...
        <tr>
            ...
            <td style="text-align: center">{{ __('messages.StoreCategory') }}</td>
            <td style="text-align: center">{{ __('messages.Phone') }}</td>
            <td style="text-align: center">{{ __('messages.Email') }}</td>
            <td style="text-align: center; display: none">{{ __('messages.OwnerName') }}</td>
            <td style="text-align: center; display: none">{{ __('messages.OwnerPhone') }}</td>
        ...
        @if (isset($MyArray))
            @foreach($MyArray as $Item)
                <tr>
                    ...
                    <td style="text-align: center"><img src="@if($Item->{'Logo'} != null) {{ $Item->{'Logo'} }} @else {{ URL::asset('images/store_icon.png') }} @endif" alt="logo"/></td>
                    <td style="text-align: center">{{ $Item->{'Name'} }}</td>
                    <td style="text-align: center">{{ $Item->{'StoreType'} }}</td>
                    <td style="text-align: center">{{ $Item->{'Phone'} }}</td>
                    <td style="text-align: center">{{ $Item->{'Email'} }}</td>
                    <td style="text-align: center; display: none">{{ $Item->{'OwnerName'} }}</td>
                    <td style="text-align: center; display: none">{{ $Item->{'OwnerPhone'} }}</td>
                    ...
                    <td style="text-align: center">@if($Item->{'EPaymentInfoId'} != null ) {{ __('messages.Available') }} @else {{ __('messages.Unavailable') }} @endif</td>
