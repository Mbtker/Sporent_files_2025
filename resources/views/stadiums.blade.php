...
@section('content')
    ...
    <h3 class="i-name">{{ __('messages.Stadiums') }}</h3>

    <!-- edit Stadiums Modal -->
    @include('modals.editStadiumsModal')
    ...
    <script>
        ...
        $(document).on('click','#SearchButton', function(){
            ...
            {
                window.location.href="{{ route('Stadiums') }}";
            }
        });
        ...
        $(document).on('click','.Details', function(){
            ...
            let currentRow=$(this).closest("tr");
            let StadiumId = currentRow.find("td:eq(0)").text(); // get current Id
        ...
        $(document).on('click','.Edit', function(){
            ...
            $("#editStadiumsModal").find('input[name="UserId"]').val(UserId);
            $("#editStadiumsModal").find('input[name="Name"]').val(Name);
            $("#editStadiumsModal").find('input[name="Phone"]').val(Phone);
            $("#editStadiumsModal").find('input[name="Email"]').val(Email);

            var Status = currentRow.find("td:eq(10)").text(); // get current Status
            $("#Status").val(Status).attr("selected","selected");

            // To remove validation
            $("#editStadiumsModal").find('small').hide();

            $("#editStadiumsModal").modal('show');
    ...
    @if(!empty(Session::get('error_stadiums')) && Session::get('error_stadiums') == 1)
        <script>
            $(function() {
                $('#editStadiumsModal').modal('show');
            });
    ...
    @if(!empty(Session::get('error_edit_stadiums')) && Session::get('error_edit_stadiums') == 3)
        <script>
