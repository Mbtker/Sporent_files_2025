...
@section('content')
    ...
    <h3 class="i-name">{{ __('messages.AdvertisingAgencies') }}</h3>

    <!-- edit AdvertisingAgencies Modal -->
    @include('modals.editAdvertisingAgenciesModal')
    ...
    <script>
        ...
        $(document).on('click','#SearchButton', function(){
            ...
            {
                jQuery.ajax({
                    url: "{{ url('/AdvertisingAgenciesSearching') }}",
                    method: 'get',
                    data: {
        ...
        $(document).on('click','.Details', function(){
            ...
            let AgencyId = currentRow.find("td:eq(0)").text(); // get current Id

            document.cookie  = "AgencyId = " + AgencyId;

            window.location.href="{{ route('AgencyDetails') }}";
        ...
        $(document).on('click','.Edit', function(){
            ...
            $("#editAdvertisingAgenciesModal").find('input[name="UserId"]').val(UserId);
            $("#editAdvertisingAgenciesModal").find('input[name="Name"]').val(Name);
            $("#editAdvertisingAgenciesModal").find('input[name="Phone"]').val(Phone);
            $("#editAdvertisingAgenciesModal").find('input[name="Email"]').val(Email);

            var Status = currentRow.find("td:eq(9)").text(); // get current Status
            $("#Status").val(Status).attr("selected","selected");

            // To remove validation
            $("#editAdvertisingAgenciesModal").find('small').hide();

            $("#editAdvertisingAgenciesModal").modal('show');
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '#All', function () {
                ...
                window.location.href="{{ route('AdvertisingAgencies') }}";
            });
    ...
    @if(!empty(Session::get('error_advertising_agencies')) && Session::get('error_advertising_agencies') == 1)
        <script>
            $(function() {
                $('#editAdvertisingAgenciesModal').modal('show');
            });
    ...
    @if(!empty(Session::get('error_edit_advertising_agencies')) && Session::get('error_edit_advertising_agencies') == 3)
        <script>
