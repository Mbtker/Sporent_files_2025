...
<div class="modal" id="editSupervisorModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('editSupervisor') }}" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="recipient-name" class="col-form-label">{{ __('messages.Location') }}: @error('Location') <small id="ErrorEmail" class="form-text text-danger">{{ $message }}</small> @enderror</label>
                        <input id="Location" type="text" class="form-control" name="Location" value="{{ old('Location') }}">
                    </div>
                    <div class="form-group">
                        <label for="recipient-name" class="col-form-label">{{ __('messages.AreaRange') }}: @error('AreaRange') <small id="ErrorEmail" class="form-text text-danger">{{ $message }}</small> @enderror</label>
                        <input id="AreaRange" type="text" class="form-control" name="AreaRange" value="{{ old('AreaRange') }}">
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-select form-control" id="Status" name="Status">
                            ...
                            <option value="0" {{ old('Status') == '1' ? "selected" : "" }}>{{ __('messages.Inactive') }}</option>
                        </select>
                    </div>
                </div>
